...
function init() {
  var pjctsWrap = document.querySelector(".projects__block");
  var pjctsElems = document.querySelectorAll(".projects__wrap");
  var buttonMore = document.querySelector("#buttonMore");
  var pjctsResized = false;
  var pjctsInitHeight;
  var pjctsResizeHeight = 0;
  var pageYnow = 0;
  $(document).ready(function () {
    pjctsWrap.addEventListener("click", function (e) {
      ...
      var linkSrc = false,
          link;
      e.target.nodeName == "IMG" ? link = e.target.parentElement : link = e.target;
      ...
      if (linkSrc) {
        ...
        setTimeout(function () {
          document.querySelector(".projects__present").classList.add("projects__present-active");
        }, 100);

        document.querySelector(".ag__button-back").onclick = function () {
          closeImg(pageYnow);
        };
  ...
  function setContentHeight() {
    switch (true) {
      case window.innerWidth > 991:
        pjctsResizeHeight = Math.ceil(pjctsElems.length / 3) * (pjctsElems[0].offsetHeight + parseInt(getComputedStyle(pjctsElems[0]).marginBottom));
        break;

      case window.innerWidth < 992 && window.innerWidth > 767:
        pjctsResizeHeight = Math.ceil(pjctsElems.length / 2) * (pjctsElems[0].offsetHeight + parseInt(getComputedStyle(pjctsElems[0]).marginBottom));
        break;

      case window.innerWidth < 768:
        pjctsResizeHeight = Math.ceil(pjctsElems.length / 1) * (pjctsElems[0].offsetHeight + parseInt(getComputedStyle(pjctsElems[0]).marginBottom));
        break;
    }

    !pjctsResized ? pjctsInitHeight = parseInt(getComputedStyle(pjctsWrap).height) : false;
  ...
  buttonMore.textContent = "Показать ещё"; // ИЗМЕНЕНИЕ КОНТЕЙНЕРА ПРИ РЕСАЙЗЕ
  ...
  function addImg(imgSrc) {
    document.querySelector(".website").insertAdjacentHTML("afterbegin", "<div class=\"projects__present-wrap\"></div><div class=\"projects__present\"><div class=\"cross cross-correct\"><span></span></div><img class=\"loader__img\" src=\"assets/img/icons/website/loading.gif\" alt=\"\"><button class=\"ag__button ag__button-back hvr-push none\">\u041D\u0430\u0437\u0430\u0434</button></div>");
    var present = document.querySelector(".projects__present");
    var loader = document.querySelector(".loader__img");
  ...
  function closeImg(pageYnow) {
    var section = document.querySelector(".website");
    var present = document.querySelector(".projects__present");
    var presentWrap = document.querySelector(".projects__present-wrap");
    ...
    setTimeout(function () {
      ...
      section.removeChild(presentWrap); // body.style.height = "auto";
      // smothScroll(pageYnow);
  ...
  function setBody() {
    var presentNow = document.querySelector(".projects__present");
    body.style.height = presentNow.offsetHeight + parseInt(getComputedStyle(presentNow).top) + 50 + "px";
  }
...
init(); // ОПРЕДЕЛЕНИЕ ВРЕМЕНИ АНИМАЦИИ

function getAnimCount(elem) {
  var timer = getComputedStyle(elem).animationIterationCount;
  return timer *= 1000;
}

function setColorOnLink() {
  var menu = document.querySelectorAll(".ag__menu_link");

  for (var i = 0; i < menu.length; i++) {
    menu[i].classList.remove("ag__menu_link-active");
  }

  if (window.location.pathname.substring(1) == "" || window.location.pathname.substring(1) == "index.html") {
...
var socNetwork = document.querySelector(".soc_networks");
var socNetworkTop = parseInt(getComputedStyle(socNetwork).top);

window.onscroll = function () {
  if (pageYOffset + 50 >= Math.ceil(socNetworkTop / 2)) {
    socNetwork.style.top = pageYOffset + Math.ceil(socNetwork.offsetWidth / 2) + 30 + "px";
  }
...
var linkInMenu = document.querySelectorAll(".ag__menu_link");

for (var i = 0; i < linkInMenu.length; i++) {
